...
static bool scan_start_tag_name(Vector *tags, TSLexer *lexer) {
    ...
    printf("%.*s\n", (int)tag_name.size, tag_name.contents);
    if (string_eq_lit(&tag_name, "world")){
        lexer->result_symbol = DEFINE_START_NAME;
...
static bool scan_end_tag_name(Vector *tags, TSLexer *lexer) {
    ...
    if (tags->size > 0 && string_eq(array_back(tags), &tag_name)) {
        ...
        if (string_eq_lit(&tag_name, "world")){
            lexer->result_symbol = DEFINE_END_NAME;
            return true;
        }
        else{
...
bool tree_sitter_gdml_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    ...
    switch (lexer->lookahead) {
        ...
        default:
            /* bool result = false; */
            /* if (valid_symbols[DEFINE_NAME] || valid_symbols[START_TAG_NAME]) { */
            /*     if(valid_symbols[DEFINE_NAME]){ */
            /*         bool result = scan_gdml_name(tags, lexer); */
            ...
            /* } */
            if (valid_symbols[START_TAG_NAME]) {
                /* result = result || scan_start_tag_name(tags, lexer); */
                return scan_start_tag_name(tags, lexer);
            }
            ...
            /* if (valid_symbols[DEFINE_NAME]) { */
            /*     result = result || scan_gdml_name(tags, lexer, 'define'); */
            /* } */
            /* return result; */
    }
